...
public final class WebhookClient implements AutoCloseable {
    ...
    public void incident(Settings settings, Incident incident, boolean restartScheduled, boolean whitelisted) { sendRepeated(settings, "**" + settings.messages().incidentTitle() + "**\nIncident: `" + incident.incidentId() + "`\nFailures: " + failures(incident) + "\nAutomatic restart attempts: " + incident.automaticRestartAttempts() + "\nWhitelist enabled: " + whitelisted + "\nRestart scheduled: " + restartScheduled + "\nLoop protection active: " + incident.restartLoopStopped() + "\nDetected: " + incident.lastDetection()); }
    ...
    private void sendRepeated(Settings s, String text) { for (int i = 0; i < s.discord().repeats(); i++) { final int n = i; executor.submit(() -> { if (n > 0) try { Thread.sleep(s.discord().delayMillis()); } catch (InterruptedException e) { Thread.currentThread().interrupt(); return; } send(s, text); }); } }
