...
public final class StartupGuardianPlugin extends org.bukkit.plugin.java.JavaPlugin implements Listener {
    ...
    @EventHandler(priority = EventPriority.HIGHEST) public void onPlayerLogin(PlayerLoginEvent event) { if (event.getResult() != PlayerLoginEvent.Result.KICK_WHITELIST || !guardian.allowsCriticalBypass(event.getPlayer())) return; event.allow(); getLogger().warning("[StartupGuardian] Allowed critical-mode bypass for " + event.getPlayer().getName()); }
